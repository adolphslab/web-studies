...
{	
	...
	$.getJSON("templates/active.txt", function(input) {
		...
		$.getJSON("templates/"+input.active+"/input.txt", function(data) { 
            ...
            //if running==embedded in input.txt
			//takes subID from URL location.search property
			//expects something like http://www.server.com/IAT/index.php?MID=12345
			//if no query is present, or if MID is empty (e.g., ?MID= ), MID defaults to 999+random string
...
{	
	...
	if (session == 7)
	{
		...
		{
		    resulttext = "<div style='text-align:center;padding:20px'>Thanks for completing this portion of the experiment! <br><br>Your completion code is: <b>y3s7h</b> <br><br>Please note down this completion code so that you can enter it into the survey in the previous window.You may now close this window/tab and continue with the rest of the experiment.</div>";
